		...
		<div class="frame">
			...
			<form action="seat.jsp">
				...
				<div class="dh-leftright">
					...
					<c:if test="${not empty param.date_id }">
					<div class="dh-time center">
						...
						<select name="time_id">
							<c:forEach var="timeDTO" items="${timelist }">
								<c:set var="sold" value="${reserveDAO.selectSold(param.show_id,param.date_id,timeDTO.time_id) }"/>
								<option value="${timeDTO.time_id }">
									${timeDTO.time_part } 남은좌석: ${seatDAO.count() - sold }
								</option>
							</c:forEach>
						</select> 
...
<script>
		const radios = document.querySelectorAll("input[name='date_id']");
		...
		radios.forEach((radio) => {
			radio.addEventListener("change", (e) => {
				...
				if(current.checked){
					var qr = '&date_id='+ date
					location.href = '${cpath}/date.jsp?show_id=${param.show_id}&month=${param.month}' + qr
				}
			});
		});
	</script>	
